...
const login = async (req, res) => {
    ...
    try {
        ...
        const accessToken = jwt.sign({ userId: user.id, username: user.username, role: user.role }, process.env.JWT_SECRET, { expiresIn: '8h' });
        const refreshToken = jwt.sign({ userId: user.id, username: user.username,role: user.role }, process.env.JWT_REFRESH_SECRET, { expiresIn: '8h' });


        await logUserAction(user.id, 'Login');
        res.json({ accessToken, refreshToken }); // ส่งทั้ง accessToken และ refreshToken กลับไปที่ client
    } catch (err) {
        console.error(err);
