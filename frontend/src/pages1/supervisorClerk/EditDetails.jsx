...
const EditDetails = () => {
  ...
  const handleEditRow = (record) => {
    ...
    setAdditionalReason(
      record.manager_reason === "อื่นๆ" ? record.additionalReason : ""
    );
  ...
  const handleSave = async () => {
    try {
      const reasonToSave =
        currentRecord.manager_reason === "อื่นๆ"
          ? additionalReason
          : currentRecord.manager_reason;

      await axios.post(
        `${config.API_URL}/dashboard/update-details/${upload_id}`,
        { ...currentRecord, manager_reason: reasonToSave }
      );

      setData((prevData) =>
        prevData.map((item) =>
          item.id === currentRecord.id
            ? { ...item, manager_reason: reasonToSave }
            : item
        )
      );
    ...
    },
    {
      ...
      render: (text, record) => {
        return record.id === editingRow ? (
          ...
          <span>{record.manager_reason}</span>
        );
      },
    },
  ];
